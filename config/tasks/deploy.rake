...
#
# Fezzik tasks for deploying and controlling the action items server.
...
namespace :fezzik do
  ...
  remote_task :start do
    ...
    run "cd #{current_path} && ruby action_items_server_control.rb start"
  end

  desc "kills the application"
  remote_task :stop do
    puts "stopping app"
    run "(kill -9 `ps aux | grep 'action_items_server' | grep -v grep | awk '{print $2}'` || true)"
  end
